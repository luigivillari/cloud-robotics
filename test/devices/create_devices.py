import os
# Contenuto base del file agente.py, senza agent_id
agente_template = '''import sys
...
broker_port = 1883
publish_topic = "/it/unime/fcrlab/robotics/register{agent_id}"
subscribe_topic = "/it/unime/fcrlab/robotics/register/token"
publish_topic_task = "/it/unime/fcrlab/robotics/task/request"
subscribe_topic_task = "/it/unime/fcrlab/robotics/task/response{agent_id}"
...
def crea_cartelle_e_script(numero_cartelle=250):
    for i in range(1, numero_cartelle + 1):
        
        cartella_nome = f"dev{i}"
        agent_id = 1000 + i
        os.makedirs(cartella_nome, exist_ok=True)
        agente_con_id = agente_template.format(agent_id=agent_id)
        agente_file_path = os.path.join(cartella_nome, "agente.py")

        with open(agente_file_path, "w") as agente_file:
            agente_file.write(agente_con_id)
        
        print(f"Creato {agente_file_path}")


crea_cartelle_e_script()
